...
#include "HashTable/Conditionals.h"
...
inline typename TEnableIf<!TUseBitwiseSwap<T>::Value>::Type Swap(T& A, T& B)
{
    T Temp = TestHashTable::Move(A);
    A = TestHashTable::Move(B);
    B = TestHashTable::Move(Temp);
}
...
struct TGuard : private CNoncopyable
{
    ...
    explicit TGuard(TArgs&&... Args)
        : _Function(TestHashTable::ForwardArg<TArgs>(Args)...)
    {
    }

    ~TGuard()
    {
        TestHashTable::Invoke(_Function);
    }
