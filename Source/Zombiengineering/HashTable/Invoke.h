...
#include "HashTable/References.h"
#include "HashTable/TypeTraits.h"
...
ULANG_FORCEINLINE auto Invoke(FuncType&& Func, ArgTypes&&... Args)
    -> decltype(TestHashTable::ForwardArg<FuncType>(Func)(TestHashTable::ForwardArg<ArgTypes>(Args)...))
{
    return TestHashTable::ForwardArg<FuncType>(Func)(TestHashTable::ForwardArg<ArgTypes>(Args)...);
}
...
ULANG_FORCEINLINE auto Invoke(ReturnType ObjType::*pdm, CallableType&& Callable)
    -> decltype(Private::DereferenceIfNecessary<ObjType>(TestHashTable::ForwardArg<CallableType>(Callable)).*pdm)
{
    return Private::DereferenceIfNecessary<ObjType>(TestHashTable::ForwardArg<CallableType>(Callable)).*pdm;
}
...
ULANG_FORCEINLINE auto Invoke(ReturnType (ObjType::*PtrMemFun)(PMFArgTypes...), CallableType&& Callable, ArgTypes&&... Args)
    -> decltype((Private::DereferenceIfNecessary<ObjType>(TestHashTable::ForwardArg<CallableType>(Callable)).*PtrMemFun)(TestHashTable::ForwardArg<ArgTypes>(Args)...))
{
    return (Private::DereferenceIfNecessary<ObjType>(TestHashTable::ForwardArg<CallableType>(Callable)).*PtrMemFun)(TestHashTable::ForwardArg<ArgTypes>(Args)...);
}
...
#define ULANG_PROJECTION(FuncName) \
    [](auto&&... Args) \
    { \
        return FuncName(TestHashTable::ForwardArg<decltype(Args)>(Args)...); \
    }
...
#define ULANG_PROJECTION_MEMBER(Type, FuncName) \
    [](auto&& Obj, auto&&... Args) \
    { \
        return Private::DereferenceIfNecessary<Type>(TestHashTable::ForwardArg<decltype(Obj)>(Obj)).FuncName(TestHashTable::ForwardArg<decltype(Args)>(Args)...); \
    }
...
namespace Private
{
    ...
    struct TIsInvocableImpl<typename TVoid<decltype(TestHashTable::Invoke(DeclVal<CallableType>(), DeclVal<ArgTypes>()...))>::Type, CallableType, ArgTypes...>
    {
        enum { Value = true };
    };
}
